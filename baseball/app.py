...
class App:
    def __init__(self):
        pygame.init()
        # 이름 입력창에서 백스페이스(등)를 꾹 누르고 있으면 한 글자씩이 아니라
        # 계속 눌린 상태로 반복 입력되도록(키 반복 없이는 pygame 이 KEYDOWN을
        # 누를 때 딱 한 번만 보냄).
...
HELP_LINES = [
    ...
    ("body", "  「홈/원정」 버튼: 입력해둔 원정·홈 라인업 전체를 통째로 맞바꿈"),
    ("gap", ""),
    ...
    ("body", "· 마지막 아웃으로 경기가 끝나면 아웃 결과 → \"경기 종료\" 순서로 표시"),
    ("body", "  (득점으로 끝나는 끝내기 승리는 예외)"),
    ("gap", ""),
    ("head", "선수 교체 (2인 모드)"),
    ("body", "· 투수 교체: 구종 선택 패널의 「교체」 버튼(다음 투구 전에만 가능)"),
    ("body", "· 대타: 우측 타자 기록 아래 「대타」 버튼(다음 투구 전에만 가능)"),
    ("body", "· 교체 전/후 선수는 경기 종료 후 「기록」 화면에 각각 별도 줄로 표시"),
    ("gap", ""),
    ("head", "경기 종료 후 기록 (2인 모드)"),
    ("body", "· 「기록」 버튼 → 상단 탭에서 타자 기록 / 투수 기록 전환"),
    ("body", "· 타자: 타순·이름·타수·득점·안타·타점·홈런·볼넷·삼진·타율"),
    ("body", "· 투수: 이름·이닝·피안타·실점·자책·4사구·삼진·피홈런·타자·타수·평균자책점"),
    ("body", "  맨 끝 칸에 승/패/세이브/홀드가 해당되는 투수에게만 표시"),
    ("body", "  승: 마지막으로 리드를 잡은 순간의 투수(동점 뒤 재역전이면 그 순간 투수)"),
    ("body", "  선발은 5이닝 미만이면 승 자격 없음 → 가장 잘 던진 구원투수가 승"),
    ("body", "  패: 결승점을 허용한 책임 투수(승계주자면 주자를 내보낸 투수)"),
    ("body", "  세이브: 3점차 이내 1이닝↑ / 동점 주자 상황 등판 / 3이닝↑ 중 하나"),
    ("body", "  홀드: 세이브 상황에 등판해 1아웃↑ 잡고 리드 지킨 채 교체"),
    ("body", "  실책으로 출루한 주자의 득점은 비자책(투수 책임 아님) 처리"),
    ("body", "  구원투수가 승계주자를 실점시키면 원래 내보낸 투수의 자책으로 기록"),
    ("gap", ""),
    ("head", "팀 색상"),
    ...
    ("body", "· 2인: 우측 타석 아래 현재 타자 기록 2줄(타수·안타·득점·타점 / 홈런·볼넷·삼진)"),
    ("body", "· 2인: 좌측 S/B/O 아래 현재 투수 기록(이닝·피안타 / 삼진·4사구 / 실점·자책 / 피홈런)"),
    ...
    ("body", "· 이름 입력: 백스페이스를 꾹 누르면 연속으로 삭제"),
    ("body", "· 일시정지: 우하단 버튼 또는 ESC → 계속하기 / 그만하기"),
    ("body", "  2인 모드는 「라인업」도 있음: 이름·좌우타를 고치고 이어서 진행(점수·기록 유지)"),
...
class LineupScene:
    def __init__(self, app, one_player, prefill_game=None, resume_scene=None):
        self.app = app
        self.one_player = one_player
        # 경기 도중 일시정지 메뉴에서 들어온 경우: 진행 중인 경기를 그대로 두고
        # 이름·좌우타만 고쳐서 이어서 한다(resume_scene = 진행 중이던 PlayScene).
        self.resume_scene = resume_scene
        if resume_scene is not None:
            prefill_game = resume_scene.game
        self.inputs = []
        self.team_inputs = []
        self.hand_btns = []
        self.pitcher_inputs = []
        if one_player:
            ...
            if prefill_game:
                ...
                if resume_scene is not None:
                    # 진행 중인 경기: 지금 마운드에 있는 투수 정보로 채운다
                    self.pitcher_right = list(prefill_game.pitcher_right)
                    pitcher_defaults = tuple(prefill_game.pitchers)
                else:
            ...
            resuming = resume_scene is not None
            self.btn_start = Button((C.WIDTH // 2 - 110, btn_y, 220, 56),
                                    "계속하기" if resuming else "경기 시작",
                                    size=26, color=C.ACCENT, hover=C.GOOD)
            self.btn_back = Button((40, btn_y + 2, 140, 52),
                                   "취소" if resuming else "닫기",
                                   size=22, color=C.PANEL, hover=C.GRAY)
            # 이미 입력한 라인업을 다시 안 치고 원정/홈을 통째로 맞바꾸는 버튼
            self.btn_swap_teams = Button((C.WIDTH - 180, btn_y + 2, 140, 52),
                                         "홈/원정", size=20,
                                         color=C.PANEL, hover=C.GRAY)
    ...
    def handle(self, events):
        mouse = pygame.mouse.get_pos()
        buttons = self._all_hand_buttons() + [self.btn_start, self.btn_back]
        if not self.one_player and self.resume_scene is None:
            buttons.append(self.btn_swap_teams)
        for b in buttons:
            b.update(mouse)
        for e in events:
            ...
            if self.btn_back.clicked(e):
                if self.resume_scene is not None:
                    self._resume_game(apply=False)
                else:
                    self.app.change_scene(MenuScene(self.app))
            elif self.btn_start.clicked(e):
                if self.resume_scene is not None:
                    self._resume_game(apply=True)
                else:
                    self._start()
            elif (not self.one_player and self.resume_scene is None
                  and self.btn_swap_teams.clicked(e)):
                self._swap_teams()

    def _swap_teams(self):
        """원정/홈에 입력해둔 라인업·좌우타·팀명·선발투수를 통째로 맞바꾼다."""
        for i in range(9):
            self.inputs[0][i].text, self.inputs[1][i].text = (
                self.inputs[1][i].text, self.inputs[0][i].text)
            self.right[0][i], self.right[1][i] = (
                self.right[1][i], self.right[0][i])
            self.hand_btns[0][i].text = "우타" if self.right[0][i] else "좌타"
            self.hand_btns[1][i].text = "우타" if self.right[1][i] else "좌타"
        self.team_inputs[0].text, self.team_inputs[1].text = (
            self.team_inputs[1].text, self.team_inputs[0].text)
        self.pitcher_inputs[0].text, self.pitcher_inputs[1].text = (
            self.pitcher_inputs[1].text, self.pitcher_inputs[0].text)
        self.pitcher_right[0], self.pitcher_right[1] = (
            self.pitcher_right[1], self.pitcher_right[0])
        self.pitcher_hand_btns[0].text = (
            "우투" if self.pitcher_right[0] else "좌투")
        self.pitcher_hand_btns[1].text = (
            "우투" if self.pitcher_right[1] else "좌투")

    def _resume_game(self, apply):
        """진행 중이던 경기로 돌아간다. apply 면 고친 이름·좌우타·팀명·투수를
        그 경기(점수·이닝·기록은 그대로)에 반영한다."""
        scene = self.resume_scene
        g = scene.game
        if apply:
            for t in range(2):
                for i, inp in enumerate(self.inputs[t]):
                    name = inp.text.strip() or self._name_defaults[t][i]
                    g.lineups[t][i] = name
                    g.batter_stats[t][i][-1]["name"] = name
                g.right[t] = list(self.right[t])
                g.team_names[t] = (self.team_inputs[t].text.strip()
                                   or self._team_defaults[t])
                pname = (self.pitcher_inputs[t].text.strip()
                         or self._pitcher_defaults[t])
                g.pitchers[t] = pname
                g.pitcher_right[t] = self.pitcher_right[t]
                if g.pitcher_stats[t]:
                    g.pitcher_stats[t][-1]["name"] = pname
                if len(g.pitcher_stats[t]) <= 1:  # 아직 선발투수 그대로
                    g.starting_pitchers[t] = pname
                    g.starting_pitcher_right[t] = self.pitcher_right[t]
            scene.refresh_batter_display()
        scene.paused = False
    ...
    def draw(self, s):
        ...
        if self.one_player:
            ...
            if self.resume_scene is None:
                self.btn_swap_teams.draw(s)
...
class PlayScene:
    def __init__(self, app, game: LiveGame):
        ...
        # 마지막 아웃으로 경기가 끝났을 때: OUT 배너를 먼저 보여준 뒤에야
        # "경기 종료" 배너로 넘어가도록(바로 결과 화면으로 점프하지 않게) 표시
        self._pending_game_over = False

        self.paused = False
        cx = C.WIDTH // 2
        # 2인 모드 일시정지: 계속하기 / 라인업(이름·좌우타 수정 후 이어서) / 그만하기
        two_p = not game.one_player
        self.btn_resume = Button(
            (cx - 120, C.HEIGHT // 2 - (58 if two_p else 8), 240, 52),
            "계속하기", size=26, color=C.GOOD, hover=C.ACCENT2)
        self.btn_lineup = Button((cx - 120, C.HEIGHT // 2 + 8, 240, 52),
                                 "라인업", size=26, color=C.BLUE, hover=C.GOOD)
        self.btn_quit_game = Button(
            (cx - 120, C.HEIGHT // 2 + (74 if two_p else 58), 240, 52),
        ...
        # 대타 버튼: 화면 우측 "타석" 기록 표시(_draw_status) 바로 아래에 둔다.
        status_bx = C.WIDTH - 118
        self.btn_batter_sub = Button(
            (status_bx - 40, 292, 80, 26),
            "대타", size=14, color=C.BLUE, hover=C.GOOD)
        ...
        self.show_batter_sub = False
        self.batter_sub_input = None
        self.batter_sub_right = True
        self.batter_sub_hand_btn = None
        self.btn_batter_sub_confirm = Button(
            (px + 40, py + 180, 170, 54), "교체", size=24,
            color=C.GOOD, hover=C.ACCENT2)
        self.btn_batter_sub_cancel = Button(
            (px + 230, py + 180, 170, 54), "닫기", size=24,
            color=C.PANEL, hover=C.GRAY)
    ...
    def _close_pitcher_sub(self, apply):
        ...
        if apply:
            ...
            if name:
                g.substitute_pitcher(team, name)
            g.pitcher_right[team] = self.pitcher_sub_right
            self.phase = P_PITCHER_CHANGE
            self.timer = 1.6

    # ------------------------------------------------ 대타
    def _open_batter_sub(self):
        pw = PITCHER_SUB_PW
        px, py = (C.WIDTH - pw) // 2, (C.HEIGHT - PITCHER_SUB_PH) // 2
        self.batter_sub_right = True
        self.batter_sub_input = TextInput((px + 40, py + 92, 220, 48),
                                          text="플레이어", max_len=8)
        self.batter_sub_hand_btn = Button(
            (px + 280, py + 92, 120, 48), "우타", size=20)
        self.show_batter_sub = True

    def _close_batter_sub(self, apply):
        self.show_batter_sub = False
        if apply:
            g = self.game
            team = g.batting_team
            slot = g.batter_idx[team] % len(g.lineups[team])
            name = self.batter_sub_input.text.strip()
            if name:
                g.substitute_batter(team, slot, name, self.batter_sub_right)
                # 교체 직후 화면에 남아있던 이전 타자 표시 캐시도 바로 갱신
                self._display_name = name
                self._display_hand_right = self.batter_sub_right
                self._display_stats_lines = g.current_batter_stats_lines()
            self.phase = P_BATTER_CHANGE
            self.timer = 1.6
    ...
    def _finish_batted(self):
        ...
        # 그 아웃으로 경기가 끝났으면, 아웃 결과 배너를 먼저 보여준 뒤에야
        # "경기 종료" 배너로 넘어간다(득점으로 끝나는 끝내기 승리는 이미
        # "끝내기 승리" 배너 자체가 결과 화면 구실을 하므로 대상이 아니다).
        if self.plan["kind"] == "out" and not wo and self.game.is_over():
            self._pending_game_over = True
    ...
    def refresh_batter_display(self):
        """라인업에서 이름·좌우타를 고치고 돌아왔을 때, 타석 표시를 새로 맞춘다
        (결과 연출 중에는 직전 타자 표시를 유지 — 다음 타석에서 갱신됨)."""
        if self.phase in (P_SELECT, P_READY, P_PITCH):
            name, order = self.game.current_batter_name()
            self._display_name = name
            self._display_order = order
            self._display_hand_right = self.game.batter_is_right()
    ...
    def _show_game_over_banner(self):
        """마지막 아웃 배너를 다 보여준 뒤, 곧바로 결과 화면으로 넘어가지
        않고 "경기 종료" 배너를 한 박자 띄운다."""
        if not self.game.one_player and self.game.game_tied:
            text = "경기 종료\n무승부"
        else:
            text = "경기 종료"
        self._show_result(text, C.ACCENT2, long_display=True)
    ...
    def handle(self, events):
        ...
        if self.paused:
            ...
            self.btn_lineup.update(mouse)
            for e in events:
                if self.btn_resume.clicked(e):
                    self.paused = False
                elif not self.game.one_player and self.btn_lineup.clicked(e):
                    self.app.change_scene(LineupScene(
                        self.app, False, resume_scene=self))
                    return
        ...
        if self.show_batter_sub:
            self.batter_sub_hand_btn.update(mouse)
            self.btn_batter_sub_confirm.update(mouse)
            self.btn_batter_sub_cancel.update(mouse)
            for e in events:
                self.batter_sub_input.handle(e)
                if self.batter_sub_hand_btn.clicked(e):
                    self.batter_sub_right = not self.batter_sub_right
                    self.batter_sub_hand_btn.text = (
                        "우타" if self.batter_sub_right else "좌타")
                elif self.btn_batter_sub_confirm.clicked(e):
                    self._close_batter_sub(apply=True)
                elif self.btn_batter_sub_cancel.clicked(e):
                    self._close_batter_sub(apply=False)
                elif e.type == pygame.KEYDOWN and e.key == pygame.K_ESCAPE:
                    self._close_batter_sub(apply=False)
                elif e.type == pygame.KEYDOWN and e.key in (
                        pygame.K_RETURN, pygame.K_KP_ENTER):
        ...
        if self.phase == P_SELECT:
            ...
            self.btn_batter_sub.update(mouse)
        for e in events:
            if self.phase == P_SELECT and self.btn_pitcher_sub.clicked(e):
                self._open_pitcher_sub()
                return
            if self.phase == P_SELECT and self.btn_batter_sub.clicked(e):
                self._open_batter_sub()
                return
    ...
    def update(self, dt):
        if self.paused or self.show_pitcher_sub or self.show_batter_sub:
            return
        ...
        if self.phase == P_READY:
            ...
            if self.timer <= 0:
                self.phase = P_SELECT
        elif self.phase == P_BATTER_CHANGE:
            self.timer -= dt
            if self.timer <= 0:
                self.phase = P_SELECT
    ...
    def draw(self, s):
        ...
        if self.phase == P_BATTER_CHANGE:
            self._draw_batter_change(s)
        if self.paused:
            self._draw_pause_overlay(s)
        if self.show_pitcher_sub:
            self._draw_pitcher_sub_modal(s)
        if self.show_batter_sub:
            self._draw_batter_sub_modal(s)
    ...
    def _draw_status(self, s):
        ...
        if not g.one_player:
            ...
            # 대타: 다음 투구가 시작되기 전(P_SELECT)에만 교체할 수 있다.
            if self.phase == P_SELECT:
                self.btn_batter_sub.draw(s)
    ...
    def _draw_pitcher_record(self, s):
        """좌측: 지금 마운드에 있는 수비 팀 투수의 이닝·피안타 / 삼진·4사구 / 실점·자책 / 피홈런."""
        g = self.game
        st = g._current_pitcher_entry(g.defending_team)
        x, y = LEFT_PANEL_X, COUNT_PANEL_Y + 100
        draw_text(s, f"투수 {st['name']}", 15, x + 10, y, C.BLACK, bold=True)
        draw_text(s, f"이닝 {_fmt_ip(st['outs'])}    피안타 {st['h']}", 15,
                  x + 10, y + 20, C.BLACK, bold=True)
        draw_text(s, f"삼진 {st['k']}    4사구 {st['bb']}", 15, x + 10, y + 40,
                  C.BLACK, bold=True)
        draw_text(s, f"실점 {st['r']}    자책 {st['er']}", 15, x + 10, y + 60,
                  C.BLACK, bold=True)
        draw_text(s, f"피홈런 {st['hr']}", 15, x + 10, y + 80, C.BLACK, bold=True)
    ...
    def _draw_pause_overlay(self, s):
        ...
        s.blit(overlay, (0, 0))
        pw, ph = 360, (230 if self.game.one_player else 300)
        px = (C.WIDTH - pw) // 2
        py = (C.HEIGHT - ph) // 2
        panel = pygame.Surface((pw, ph), pygame.SRCALPHA)
        panel.fill((18, 22, 32, 245))
    ...
    def _draw_batter_change(self, s):
        g = self.game
        overlay = pygame.Surface((C.WIDTH, C.HEIGHT), pygame.SRCALPHA)
        overlay.fill((0, 0, 0, 130))
        s.blit(overlay, (0, 0))
        ...
        s.blit(panel, rect.topleft)
        pygame.draw.rect(s, C.BLUE, rect, width=4, border_radius=12)
        draw_text(s, "대타", 46, rect.centerx, rect.centery - 18,
                  C.BLUE, center=True, bold=True)
        team = g.batting_team
        slot = g.batter_idx[team] % len(g.lineups[team])
        hand = "우타" if g.right[team][slot] else "좌타"
        draw_text(s, f"{g.lineups[team][slot]} ({hand})", 24,
                  rect.centerx, rect.centery + 28, C.WHITE, center=True, bold=True)

    def _draw_batter_sub_modal(self, s):
        overlay = pygame.Surface((C.WIDTH, C.HEIGHT), pygame.SRCALPHA)
        overlay.fill((0, 0, 0, 165))
        s.blit(overlay, (0, 0))
        pw, ph = PITCHER_SUB_PW, PITCHER_SUB_PH
        ...
        s.blit(panel, (px, py))
        pygame.draw.rect(s, C.ACCENT2, (px, py, pw, ph), width=3, border_radius=14)
        draw_text(s, "대타", 32, px + pw // 2, py + 40, C.WHITE,
                  center=True, bold=True)
        self.batter_sub_input.draw(s)
        self.batter_sub_hand_btn.draw(s)
        self.btn_batter_sub_confirm.draw(s)
        self.btn_batter_sub_cancel.draw(s)
...
# ============================================================ 게임 종료
def _fmt_ip(outs):
    """투수 이닝 표기: 0, 1/3, 2/3, 1, 1 1/3, 1 2/3 ..."""
    whole, rem = divmod(outs, 3)
    frac = {0: "", 1: "1/3", 2: "2/3"}[rem]
    if whole == 0:
        return frac or "0"
    return f"{whole} {frac}" if frac else str(whole)


PITCHER_DECISION_LABELS = {"W": "승", "L": "패", "S": "세이브", "H": "홀드"}
...
class GameOverScene:
    def __init__(self, app, game: LiveGame):
        ...
        self.btn_tab_pitcher = Button((C.WIDTH // 2 - 150, 60, 140, 32),
                                      "투수 기록", size=17, color=C.PANEL,
                                      hover=C.GRAY)
        self.btn_tab_batter = Button((C.WIDTH // 2 + 10, 60, 140, 32),
                                     "타자 기록", size=17, color=C.PANEL,
                                     hover=C.GRAY)
    ...
    def handle(self, events):
        ...
        if self.show_stats:
            ...
            self.btn_tab_batter.update(mouse)
            self.btn_tab_pitcher.update(mouse)
            for e in events:
                if self.btn_stats_close.clicked(e):
                    self.show_stats = False
                elif self.btn_tab_batter.clicked(e):
                    self.stats_tab = "batter"
                elif self.btn_tab_pitcher.clicked(e):
                    self.stats_tab = "pitcher"
            return
    ...
    def _draw_boxscore(self, s):
        """기록 화면: 상단 탭으로 타자 기록 / 투수 기록을 전환해서 보여준다."""
        s.fill(C.DARK_PANEL)
        title = "타자 기록" if self.stats_tab == "batter" else "투수 기록"
        draw_text(s, title, 34, C.WIDTH // 2, 34, C.WHITE,
                  center=True, bold=True)

        self.btn_tab_batter.color = C.GOOD if self.stats_tab == "batter" else C.PANEL
        self.btn_tab_pitcher.color = C.GOOD if self.stats_tab == "pitcher" else C.PANEL
        self.btn_tab_batter.draw(s)
        self.btn_tab_pitcher.draw(s)

        if self.stats_tab == "batter":
            self._draw_batter_table(s)
        else:
            self._draw_pitcher_table(s)

        self.btn_stats_close.draw(s)

    def _draw_batter_table(self, s):
        """양팀 타자 기록표(타수·득점·안타·타점·홈런·볼넷·삼진·타율).

        대타로 교체된 타순은 같은 번호 아래에 교체 전/후 선수의 기록을
        각각 별도 줄로 보여준다(교체 전 선수 몫, 대타 몫이 서로 안 섞인다).
        """
        g = self.game
        col_w = (38, 82, 38, 38, 38, 38, 38, 38, 38, 56)
        headers = ("타순", "이름", "타수", "득점", "안타", "타점", "홈런", "볼넷", "삼진", "타율")
        table_w = sum(col_w)
        gap = 36
        ...
        for ti in range(2):
            ...
            ry = hy + 22
            for slot in range(9):
                for si, st in enumerate(g.batter_stats[ti][slot]):
                    if st["ab"] > 0:
                        avg = st["h"] / st["ab"]
                        avg_str = f"{avg:.3f}".lstrip("0") if avg < 1 else f"{avg:.3f}"
                    else:
                        avg_str = "-"
                    order = f"{slot + 1}" if si == 0 else ""
                    vals = (order, st["name"], st["ab"], st["r"], st["h"],
                            st["rbi"], st["hr"], st["bb"], st["k"], avg_str)
                    cx = tx
    ...
    def _draw_pitcher_table(self, s):
        """양팀 투수 기록표(이닝·피안타·실점·자책·4사구·삼진·피홈런·타자·타수·평균자책점·결과).

        투수 교체가 일어나면 교체 전/후 투수의 기록이 각자 별도 줄로 남는다
        (자책점도 실제로 그 주자를 내보낸 투수 몫으로 붙는다 — 구원투수가
        승계주자를 실점시켜도 원래 투수의 자책으로 잡힌다). 맨 끝 칸은 승/패/
        세이브/홀드가 해당되는 투수에게만 표시되고(헤더 라벨 없이) 나머지는
        빈칸이다. 칸 수가 많아 두 팀을 가로로 나란히 두면 너무 좁아지므로,
        한 팀씩 위아래로 쌓아 화면 폭을 최대한 활용한다.
        """
        g = self.game
        col_w = (86, 46, 52, 46, 46, 52, 46, 52, 46, 46, 68, 46)
        headers = ("이름", "이닝", "피안타", "실점", "자책", "4사구", "삼진",
                   "피홈런", "타자", "타수", "평균자책점", "")
        table_w = sum(col_w)
        start_x = (C.WIDTH - table_w) // 2
        row_h = 22
        y = 100

        for ti in range(2):
            draw_text(s, g.team_names[ti], 20, C.WIDTH // 2, y,
                      C.WHITE, center=True, bold=True)
            hy = y + 26
            cx = start_x
            for w, h in zip(col_w, headers):
                draw_text(s, h, 14, cx + w // 2, hy, C.LIGHT_GRAY,
                          center=True, bold=True)
                cx += w
            pygame.draw.line(s, C.GRAY, (start_x, hy + 12),
                             (start_x + table_w, hy + 12), 1)

            ry = hy + 20
            for st in g.pitcher_stats[ti]:
                ip_str = _fmt_ip(st["outs"])
                if st["outs"] > 0:
                    era_str = f"{st['er'] * 27 / st['outs']:.2f}"
                else:
                    era_str = "-"
                dec_str = PITCHER_DECISION_LABELS.get(st["decision"], "")
                vals = (st["name"], ip_str, st["h"], st["r"], st["er"],
                        st["bb"], st["k"], st["hr"], st["bf"], st["ab"],
                        era_str, dec_str)
                cx = start_x
                for w, val in zip(col_w, vals):
                    draw_text(s, str(val), 14, cx + w // 2, ry, C.WHITE,
                              center=True)
                    cx += w
                ry += row_h
            y = ry + 30
